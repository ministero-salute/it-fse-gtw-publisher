...
public class KafkaConsumerPropertiesCFG {
    ...
    public Optional<EventStatusEnum> asExceptionType(Exception e) {
        ...
        // Identify
        if (deadLetterExceptions.contains(identifier))
            status = EventStatusEnum.BLOCKING_ERROR;
        if (temporaryExceptions.contains(identifier))
            status = EventStatusEnum.NON_BLOCKING_ERROR;
        // Return
